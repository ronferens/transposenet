"""
Credit: Projective Manifold Gradient Layer for Deep Rotation Regression (CVPR 2022)
Reference: https://github.com/JYChen18/RPMG/blob/main/utils/tools.py
"""
...
class simple_RPMG(torch.autograd.Function):
    ...
    def forward(ctx, in_nd, tau, lam, weight=1):
        ...
        if proj_kind == 6:
            r0 = compute_rotation_matrix_from_ortho6d(in_nd)
        elif proj_kind == 9:
            r0 = symmetric_orthogonalization(in_nd)
        elif proj_kind == 4:
            r0 = compute_rotation_matrix_from_quaternion(in_nd)
        elif proj_kind == 10:
            r0 = compute_rotation_matrix_from_10d(in_nd)
        else:
    ...
    def backward(ctx, grad_in):
        ...
        if proj_kind == 6:
            ...
            SVD_proj = compute_SVD_nearest_Mnlsew(in_nd.reshape(-1, 3, 3), r_new)
            gradient_nd = in_nd - SVD_proj + lam * (SVD_proj - r_new.reshape(-1, 9))
        elif proj_kind == 4:
            q_1 = compute_quaternions_from_rotation_matrices(r_new)
            q_2 = -q_1
            normalized_nd = normalize_vector(in_nd)
            q_new = torch.where(
            ...
            qg = compute_quaternions_from_rotation_matrices(r_new)
            new_x = compute_nearest_10d(in_nd, qg)
            reg_A = torch.eye(4, device=qg.device)[None].repeat(qg.shape[0], 1, 1) - torch.bmm(qg.unsqueeze(-1),
                                                                                               qg.unsqueeze(-2))
            reg_x = convert_A_to_Avec(reg_A)
